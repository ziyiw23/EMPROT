...
def load_sequence(data_root: str, split: str, protein_id: Optional[str], seed: int) -> Tuple[str, np.ndarray, np.ndarray]:
    ...
    # Handle variable number of residues per frame by padding with -1
    if len(Ys) > 0:
        max_res = max(arr.shape[0] for arr in Ys)
        padded_Ys = []
        for arr in Ys:
            if arr.shape[0] < max_res:
                pad = np.full((max_res - arr.shape[0],), -1, dtype=arr.dtype)
                padded_Ys.append(np.concatenate([arr, pad]))
            else:
...
def plot_residue_trajectories_pretty(traj_name: str,
                                     times_ns: np.ndarray,
                                     Y_gt: np.ndarray,
                                     Y_pred: np.ndarray,
                                     residue_indices: List[int],
                                     out_path: Path,
                                     pred_label: Optional[str] = 'Autoregressive Prediction') -> None:
    _maybe_set_plot_style()
    ...
    for i, (ax, ridx) in enumerate(zip(axes, ridxs)):
        ...
        ax.plot(t_rel, pr, color='#ff7f0e', linewidth=1.8, label=str(pred_label or 'Autoregressive Prediction'))
...
def plot_residue_visitation_bars(traj_name: str,
    ...
    for i, ridx in enumerate(ridxs):
        ...
        width = 0.25 if Y_baseline is not None else 0.4
        
        ax.bar(x - width, h_gt[order], width=width, label='GT')
        ax.bar(x, h_pr[order], width=width, label='Pred')
        
        if Y_baseline is not None:
            base = Y_baseline[:, ridx]
            h_base = _safe_hist(base, num_classes)
            ax.bar(x + width, h_base[order], width=width, label=baseline_label)
...
def plot_histograms(gt: np.ndarray, pred: np.ndarray, out_path: Path, top_k: int = 30, 
    ...
    _maybe_set_plot_style()
    # Infer num_classes from data if not provided (safe for visualization of top-k)
    vals = [gt, pred]
    if baseline is not None:
        vals.append(baseline)
    c_max = int(max(np.max(v) if v.size > 0 else 0 for v in vals))
    num_classes = c_max + 1
    ...
    width = 0.25 if baseline is not None else 0.4
    
    ax.bar(x - width, gt_vis[order], width=width, label='GT')
    ax.bar(x, pr_vis[order], width=width, label=pred_label)
    
    if base_vis is not None:
        ax.bar(x + width, base_vis[order], width=width, label=baseline_label)
