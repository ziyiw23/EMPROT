...
def _stage_embed(pdb_file: Optional[str], history_k: int, residue_meta: Optional[List[Dict[str, Any]]] = None) -> Generator:
    logs: List[str] = []
    if not pdb_file:
    ...
    try:
        status_text = "**Status:** Embedding protein with COLLAPSE <span class='dots'></span>"
        yield (
            gr.update(visible=False),
            status_text,
            "",
        ...
        state["residue_meta"] = residue_meta or []
...
def _stage_run_inference(
    ...
    delta_t: float,
    history_k: int,
    residue_filter_text: Optional[str],
    aa_filter_values: Optional[List[str]],
    ...
    manual_ids = _parse_residue_filter_text(residue_filter_text)
    aa_filters = [aa.strip().upper() for aa in (aa_filter_values or []) if aa]
    active_residue_meta = embed_state.get("residue_meta") or residue_meta or []
    if manual_ids or aa_filters:
        filter_desc = []
        if manual_ids:
            filter_desc.append(f"indices={manual_ids}")
        if aa_filters:
            filter_desc.append(f"aa={aa_filters}")
        logs.append(f"Residue filters: {'; '.join(filter_desc)}")
    logs.append("Launching EMPROT quick rollout from demo LMDB…")
    yield (
        gr.update(),
        gr.update(),
        "**Inference:** Running EMPROT rollout & generating plots <span class='dots'></span>",
        None,
        "",
        _format_logs_block(logs),
        embed_state,
    ...
    try:
        ...
        summary_table, rollout_fig, gallery_html, pipeline_logs = _render_pipeline_outputs(
            result,
            delta_t,
            residue_meta=active_residue_meta,
            residue_ids=manual_ids,
            aa_filter=aa_filters,
        )
        ...
        yield (
            gr.update(),
            gr.update(),
            "**Inference:** Inference complete, results below.",
            rollout_fig,
            gallery_html,
            f"```\n{combined_logs}\n```",
            embed_state,
        ...
        yield (
            gr.update(),
            gr.update(),
            "**Inference:** Error during inference — see Logs for details.",
            None,
            "",
            _format_logs_block(logs),
            embed_state,
